...
// кода индекс не получают.
//
// Порога «правильного для всех» нет: редакторы вроде Cursor индексируют всё
// подряд, а агенты с grep-первым поиском (Claude Code, Cline) не индексируют
// ничего. Наш агент ищет grep'ом и на средней базе справляется сам, а индекс
// стоит долгой прогонки эмбеддингов через ollama и устаревает при каждом
// коммите. Поэтому «авто» включается только на по-настоящему большой базе:
// 5 МБ кода — это порядка 100–150 тысяч строк, где имя сущности уже не
// угадывается по памяти и поиск по смыслу начинает окупаться. Кому нужно
// раньше — есть режим «всегда».
const autoThresholdBytes = 5_000_000
