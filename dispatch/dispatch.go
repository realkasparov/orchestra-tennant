...
var (
	...
	// ErrPaused — подтверждение пришло к заданию, которое человек уже
	// остановил: пауза обогнала приём. Исполнителю нужно сказать «стой»,
	// а не считать задание идущим.
	ErrPaused = errors.New("задание остановлено человеком")
)
...
func (d *Dispatcher) Accept(jobID string, deviceID int64) error {
	...
	if j.state == JobPaused {
		// Человек остановил задание, пока предложение было в пути (или
		// подтверждение повторилось после паузы). Принять его — значит
		// запустить то, что уже остановлено.
		return ErrPaused
	}
	j.state = JobRunning
...
func (d *Dispatcher) Reject(jobID string, deviceID int64, retryable bool) error {
	...
	if !retryable {
		...
		if j.reoffer || j.target == deviceID {
			// Своя же машина не понимает план: ни ей, ни другим. То же —
			// машина проекта: задание предназначено только ей, и без неё
			// оно осталось бы в очереди навсегда без объяснения. Снимаем и
